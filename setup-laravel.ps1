...
if (Test-Path "composer.json") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "[FAIL] Failed to install Composer dependencies" -ForegroundColor Red
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "[FAIL] Failed to generate application key" -ForegroundColor Red
}
...
if (Test-Path "assets") {
    if (!(Test-Path "public/assets")) {
        ...
        Write-Host "[OK] Assets copied to public/assets" -ForegroundColor Green
    } else {
        Write-Host "[OK] Assets already in public folder" -ForegroundColor Yellow
    }
} else {
    Write-Host "[WARNING] Assets folder not found - will skip for now" -ForegroundColor Yellow
}
...
if ($response -eq "y" -or $response -eq "Y") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "[FAIL] Migration failed. Please check your database configuration" -ForegroundColor Red
    }
} else {
    Write-Host "[WARNING] Skipping database migration. Run 'php artisan migrate' later" -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "[FAIL] Asset build failed" -ForegroundColor Red
}
